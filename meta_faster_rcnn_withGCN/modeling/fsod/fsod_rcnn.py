...
This is a copy of fsod_rcnn of metafasterrcnn with the added module ss_edge + GCN from QA that enchances class prototypes!
...
##############################################################
    ...
    def load_prototype_voc(self, voc_split_id):
        ...
        for cls in support_df['category_id'].unique():
            ...
            for i in range(1,support_images.tensor.size(dim=0)):
                ...
                support_features['res4'] = torch.cat((support_features['res4'],support_features_single['res4']),dim = 0)
                #print("Suppport features size after image ",i, ":   ",support_features['res4'].shape)
    ...
    def _process_per_class(self, pos_detector_proposals, query_features, pos_support_box_features, image_size):
        ...
        return box_features_initial[-1,:].unsqueeze(0), box_features_initial[:-2, :]
    ...
    def forward(self, batched_inputs):
        ...
        if self.invoke_count == 0:
            print("calculating base class prototypes")
        ...
        # support branches
        support_bboxes_ls = []
        #For each batched input, get its bboxes and append them to support_bboxes_ls
        for item in batched_inputs:
            bboxes = item['support_bboxes']
            #for each box create a detectron2.structures class Boxes and push to GPU?
            for box in bboxes:
        ...
        #box_features = self.pooler( [features[f] for f in self.in_features], boxes)
        #print("Roi AlignV2, support feature and support bboxes")
...
###################################################################################
            ...
            #print("position begin, position end:", pos_begin, pos_end)

            #Take the positive support box featutes and average them
...
#######################################################################################################
            ...
            #print("pos_detector_proposals:",pos_detector_proposals)
            #print("images.image_sizes[i]",images.image_sizes[i])
...
#################################################################################################################
                ...
                #neg_pred_class_logits_tmp, neg_pred_proposal_deltas_tmp = self.roi_heads(neg_support_box_features, neg_proposals)
